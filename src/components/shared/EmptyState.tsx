...
export function EmptyState({ title, description, icon = "generic", action }: EmptyStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 px-4 text-center rounded-2xl border border-white/5 bg-[#09090b] relative overflow-hidden">
      ...
      <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl bg-white/5 border border-white/10 mb-6 shadow-[0_0_30px_rgba(124,58,237,0.15)]">
...
export function ErrorState({ message, onRetry }: ErrorStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-24 px-4 text-center rounded-2xl border border-red-500/10 bg-[#09090b] relative overflow-hidden">
      ...
      <h3 className="text-xl font-bold mb-2 tracking-tight text-white">Something went wrong</h3>
      ...
      {onRetry && (
        <button
          onClick={onRetry}
          className="relative z-10 inline-flex items-center gap-2 rounded-lg border border-white/10 px-5 py-2.5 text-sm font-medium hover:bg-white/5 transition-colors active:scale-[0.98]"
        >
          Try again
        </button>
      )}
